...
namespace {
    ...
    anonys::State* handle(Me& me, evt::sys::Calibrate& event) {
        return nullptr;
    }

    anonys::State* handle(Me& me, const evt::sys::Acknowledge& event) {
        return nullptr;
    }
...
namespace anonys_0_266 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::sys::Calibrate>().id:
            return handle(me, *static_cast<evt::sys::Calibrate*>(event.pData));
        case anonys::getEventId<evt::sys::Acknowledge>().id:
            return handle(me, *static_cast<evt::sys::Acknowledge*>(event.pData));
