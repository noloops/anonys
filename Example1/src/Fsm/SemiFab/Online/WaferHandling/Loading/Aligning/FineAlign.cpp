...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::wafer::WaferAligned& event) {
        return nullptr;
    }

    anonys::State* handle(Me& me, const evt::wafer::WaferFlipped& event) {
        return nullptr;
    }
...
namespace anonys_0_14 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::wafer::WaferAligned>().id:
            return handle(me, *static_cast<evt::wafer::WaferAligned*>(event.pData));
        case anonys::getEventId<evt::wafer::WaferFlipped>().id:
            return handle(me, *static_cast<evt::wafer::WaferFlipped*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pRobot, *terminals.pWaferSensor } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
