...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::xfer::LoadLockPumped& event) {
        return nullptr;
    }

    anonys::State* handle(Me& me, const evt::xfer::LoadLockVented& event) {
        return nullptr;
    }
...
namespace anonys_0_18 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::xfer::LoadLockPumped>().id:
            return handle(me, *static_cast<evt::xfer::LoadLockPumped*>(event.pData));
        case anonys::getEventId<evt::xfer::LoadLockVented>().id:
            return handle(me, *static_cast<evt::xfer::LoadLockVented*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pRobot, *terminals.pLoadLock } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
