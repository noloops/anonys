...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::xfer::ConveyorTick& event) {
        return nullptr;
    }
...
namespace anonys_0_20 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::xfer::ConveyorTick>().id:
            return handle(me, *static_cast<evt::xfer::ConveyorTick*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pConveyor } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
