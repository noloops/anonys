...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::etch::OverEtchDone& event) {
        return nullptr;
    }
...
namespace anonys_0_61 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::etch::OverEtchDone>().id:
            return handle(me, *static_cast<evt::etch::OverEtchDone*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pPlasmaGen, *terminals.pGasCtrl } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
