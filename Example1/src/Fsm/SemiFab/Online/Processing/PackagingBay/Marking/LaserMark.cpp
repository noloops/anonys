...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::pkg::MarkDone& event) {
        return nullptr;
    }
...
namespace anonys_0_221 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::pkg::MarkDone>().id:
            return handle(me, *static_cast<evt::pkg::MarkDone*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pMarker } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
