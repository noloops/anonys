...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::pkg::PackDone& event) {
        return nullptr;
    }
...
namespace anonys_0_223 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::pkg::PackDone>().id:
            return handle(me, *static_cast<evt::pkg::PackDone*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pPanel, *terminals.pRobot } };
            terminals.pSorter = &me.sorter;
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
