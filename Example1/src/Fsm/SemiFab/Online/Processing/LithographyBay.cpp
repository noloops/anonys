...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::litho::ExposureStart& event) {
        return nullptr;
    }
...
namespace anonys_0_27 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::litho::ExposureStart>().id:
            return handle(me, *static_cast<evt::litho::ExposureStart*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            ...
            terminals.pScanner = &me.scanner;
            terminals.pMaskStage = &me.maskStage;
            terminals.pLaserSource = &me.laserSource;
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
            exit(me);
            me.~Me();
            terminals.pScanner = nullptr;
            terminals.pMaskStage = nullptr;
            terminals.pLaserSource = nullptr;
        }
    }
