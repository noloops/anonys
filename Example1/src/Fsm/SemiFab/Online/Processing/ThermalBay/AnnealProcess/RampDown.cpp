...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::thermal::CooldownDone& event) {
        return nullptr;
    }
...
namespace anonys_0_135 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::thermal::CooldownDone>().id:
            return handle(me, *static_cast<evt::thermal::CooldownDone*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pFurnace, *terminals.pCoolingSys, *terminals.pTempCtrl } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
