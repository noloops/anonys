...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::dep::FilmThicknessOk& event) {
        return nullptr;
    }

    anonys::State* handle(Me& me, evt::dep::GasMixSet& event) {
        return nullptr;
    }
...
namespace anonys_0_75 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::dep::FilmThicknessOk>().id:
            return handle(me, *static_cast<evt::dep::FilmThicknessOk*>(event.pData));
        case anonys::getEventId<evt::dep::GasMixSet>().id:
            return handle(me, *static_cast<evt::dep::GasMixSet*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pHeater, *terminals.pGasMixer, *terminals.pShowerhead, *terminals.pThicknessMon } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
