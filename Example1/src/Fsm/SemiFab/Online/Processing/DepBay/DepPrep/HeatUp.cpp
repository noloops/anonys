...
namespace {
    ...
    anonys::State* handle(Me& me, evt::dep::TemperatureReached& event) {
        return nullptr;
    }
...
namespace anonys_0_70 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::dep::TemperatureReached>().id:
            return handle(me, *static_cast<evt::dep::TemperatureReached*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pHeater } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
