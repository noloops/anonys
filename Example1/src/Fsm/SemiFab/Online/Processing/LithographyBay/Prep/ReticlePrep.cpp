...
namespace {
    ...
    anonys::State* handle(Me& me, const evt::litho::ReticleSwap& event) {
        return nullptr;
    }
...
namespace anonys_0_31 {
    anonys::State* handleEvent(void* pMembers, anonys::Event& event) {
        Me& me{ *static_cast<Me*>(pMembers) };
        switch (event.eventId.id) {
        case anonys::getEventId<evt::litho::ReticleSwap>().id:
            return handle(me, *static_cast<evt::litho::ReticleSwap*>(event.pData));
    ...
    void liveCycle(bool create, void* pTerminals, void* pMembers) {
        ...
        if (create) {
            Me& me{ *::new (pMembers) Me{ *terminals.pTimer, *terminals.pStd, *terminals.pMaskStage } };
            enter(me);
        }
        else {
            Me& me{ *static_cast<Me*>(pMembers) };
